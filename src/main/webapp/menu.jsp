...
<meta name="description"
	content="Explore the full menu of <%=restName%> on Platter. Browse dishes, ratings, and order your favourites.">
...
<body>
	...
	<header class="navbar" id="mainNavbar">
		<div class="navbar-container">
			...
			<form class="nav-search-bar" action="restaurants" method="get"
				role="search">
				<i class="fa-solid fa-magnifying-glass search-icon"></i> <input
					type="text" name="q" id="navSearchInput"
					placeholder="Search restaurants or cuisines..." autocomplete="off">
			...
			<div class="nav-actions">
				...
				</a> <a href="cart" class="nav-btn-icon" aria-label="Cart" title="Cart">
					<i class="fa-solid fa-bag-shopping"></i> <span
					class="badge badge-secondary cart-badge-count"
	...
	<div class="mobile-drawer" id="mobileDrawer">
		...
		<div class="drawer-body">
			<nav class="drawer-nav">
				<a href="home" class="drawer-nav-item"><i
					class="fa-solid fa-house"></i> Home</a> <a href="restaurants"
					class="drawer-nav-item"><i class="fa-solid fa-utensils"></i>
					Restaurants</a> <a href="favorites" class="drawer-nav-item"><i
	...
	<section class="menu-hero">
		...
		<img src="<%=restaurant.getImagePath()%>"
			alt="<%=restaurant.getName()%>" class="menu-hero-image">
		<%
		} else {
		%>
		<img src="images/restaurants/pizza.jpg" alt="Restaurant"
		...
		<div class="menu-hero-content">
			...
			<div class="menu-breadcrumb">
				<a href="restaurants" class="breadcrumb-link"> <i
					class="fa-solid fa-arrow-left"></i> All Restaurants
				</a> <span class="breadcrumb-sep">/</span> <span
					class="breadcrumb-current"> <%=restName%>
				</span>
			</div>
			...
			<div class="menu-hero-top-badges">
				...
				%>
				<span class="hero-open-badge open"> <i
					class="fa-solid fa-circle" style="font-size: .55rem;"></i> Open Now
				</span>
				<%
				} else {
				%>
				<span class="hero-open-badge closed"> <i
					class="fa-solid fa-circle" style="font-size: .55rem;"></i> Closed
				</span>
				<%
				}
				%>
				...
				%>
				<span class="hero-top-rated-badge"> <i
				...
				%>
				<span class="hero-free-del-badge"> <i
					class="fa-solid fa-truck-fast"></i> Free Delivery
				</span>
				<%
				}
				%>
			...
			<div class="menu-cuisine-tags">
				...
				%>
				<span class="cuisine-tag-pill"> <%=c.trim()%>
				</span>
				<%
				}
				%>
			...
			<div class="menu-hero-stats">
				...
				<div class="hero-stat-divider"></div>
				<div class="hero-stat-item">
					<i class="fa-solid fa-indian-rupee-sign"></i> &#8377;<%=restaurant.getCostForOne()%>
					for two
				</div>
			</div>
			<%
	...
	<div class="menu-content-area" id="menuContentArea">
		...
		java.util.LinkedHashMap<String, java.util.List<Dish>> sectionMap = new java.util.LinkedHashMap<String, java.util.List<Dish>>();
		...
		if (anyDishes) {
			...
			sec = "";
				...
				if (sec.contains("main"))
			sectionMap.get("Main Course").add(d);
				else if (sec.contains("rice") || sec.contains("biryani"))
			sectionMap.get("Rice & Biryani").add(d);
				else if (sec.contains("wrap"))
			sectionMap.get("Wraps").add(d);
				else if (sec.contains("dessert") || sec.contains("sweet"))
			sectionMap.get("Desserts").add(d);
				else
			sectionMap.get("Recommended For You").add(d);
			}
		}
		%>
		...
		<div class="menu-sections-wrap" id="menuDishesGrid">
			...
			<div class="menu-section-block">
				<div class="menu-section-header">
					...
					<span class="menu-section-count"> <%=sectionDishes.size()%>
						item<%=sectionDishes.size() != 1 ? "s" : ""%>
					</span>
				</div>
				<div class="menu-section-dishes">
					...
					for (Dish d : sectionDishes) {
						...
						case "trending" :
							tagBadgeClass = "trending";
							tagIcon = "fa-fire";
							break;
						case "bestseller" :
							tagBadgeClass = "bestseller";
							tagIcon = "fa-star";
							break;
						case "new" :
							tagBadgeClass = "new-tag";
							tagIcon = "fa-sparkles";
							break;
						case "most loved" :
							tagBadgeClass = "most-loved";
							tagIcon = "fa-heart";
							break;
						default :
							tagBadgeClass = "trending";
					...
					<div class="menu-dish-card scroll-reveal"
						...
						data-tag="<%=tagLower%>" data-rating="<%=d.getRating()%>"
						data-price="<%=d.getPrice()%>" id="dishCard-<%=d.getDishId()%>">
						...
						<div class="dish-img-wrap">
							...
							if (tag != null && !tag.isEmpty()) {
							%>
							<span class="dish-tag-badge <%=tagBadgeClass%>"> <i
								class="fa-solid <%=tagIcon%>"></i> <%=tag%>
							</span>
							<%
							}
							%>
						...
						<div class="dish-body">
							<div class="dish-name-row">
								<div class="dish-type-indicator <%=vegClass%>"
									title="<%=d.isVeg() ? "" : "Non-Vegetarian"%>">
									<span class="dot"></span>
								</div>
								<h3 class="dish-title">
									<%=d.getName()%>
							...
							<div class="dish-meta-row">
								<div class="dish-rating-chip">
									<i class="fa-solid fa-star"></i> <span> <%=d.getRating()%>
									</span>
								</div>
								<%
								if (d.getCalories() != null && !d.getCalories().isEmpty()) {
								%>
								<span class="dish-calories-tag"> <%=d.getCalories()%>
								</span>
								<%
								}
								%>
						...
						<div class="dish-row-right">
							...
							<button class="<%=favDishClass%>"
								data-dish-id="<%=d.getDishId()%>" aria-label="<%=favDishTitle%>"
								title="<%=favDishTitle%>">
		...
		<div class="menu-cart-bar" id="menuCartBar">
			...
			<button class="cart-bar-view-btn"
	...
	<footer class="premium-footer scroll-reveal">
		<div class="footer-container">
			...
			<p class="footer-bottom-text">By continuing past this page, you
				...
				owners. 2008-2026 &copy; Platter&trade; Ltd. All rights reserved.</p>
		</div>
	</footer>
